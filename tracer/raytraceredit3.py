...
t = np.vstack(tarray)
Rdisk = 10 # radius of the disk
M = 1     # normalized for ease
a = 1.0 # angmom in theta direction
#initial conditions spherical coords(at point of observer)
r0 = 50*M
theta0 = np.pi/2 + 0.1
phi0 = 0 #np.pi/2
pixelcoeff = 0.015 # similar to aperture and set as 0.025 or 0.05 when r0=100M. Scale accordingly - larger r0 smaller pixelcoeff

#figures
fig = plt.figure()
ax = fig.add_subplot(111, projection="3d")
density = 10 # total number of pixels is density by density
...
for l in pixelspace: #np.arange(0,density): #multiple rays
    ...
    def solver(z,tao):
        ...
        pr = z[3]
        ptheta = z[4]
        drdtao = rprime(r, pr, theta)
        dthetadtao = thetaprime(r,theta,ptheta)
        dphidtao = phiprime(r, theta)
        dprdtao = prprime(r, pr, theta)
        dpthetadtao = pthetaprime(r, theta)
