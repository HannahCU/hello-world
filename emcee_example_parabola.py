...
""" This is an adaptation for a parabola"""
...
yerr = 0.1+5.5*np.random.rand(N) #trvially y error
y = a_true*x**2 + m_true*x + b_true #linear line using variables defined above
y += np.abs(f_true*y) * np.random.randn(N) #reproducing some physical process, ie randomfluctuations, randn produces std normal dist of shape array of N samples
y += yerr * np.random.randn(N) 


#true value plot
xfine = np.linspace(0,np.max(x),100)
plt.plot(xfine, a_true*xfine**2 + m_true*xfine + b_true, 'b-', label="True Values")


#curvefit plot
def parab(x, a, b, c):
    return a*x**2 + b*x + c
popt, pcov = op.curve_fit(parab, x, y, sigma=yerr)
plt.errorbar(x,y,yerr, fmt='.k',capsize = 1, lw = 0.5)
plt.plot(xfine, parab(xfine, popt[0], popt[1], popt[2]), 'r-', label="Curve Fit")
print("From curvefit a = ", popt[0], "+/-", pcov[0,0]**0.5)
print("m = ", popt[1], "+/-", pcov[1,1]**0.5)
print("b = ", popt[2], "+/-", pcov[2,2]**0.5)
...
A = np.vstack((np.ones_like(x), x, x**2)).T      #stack vertically and transpose, added quadratic column
C = np.diag((yerr * yerr))                 #covaiance matrix, values of error squared, diagonal through matrix
cov = np.linalg.inv(np.dot(A.T, np.linalg.solve(C,A)))
b_ls, m_ls, a_ls = np.dot(cov, np.dot(A.T, np.linalg.solve(C, y))) #add to bottom of matrix 
plt.plot(xfine, a_ls*xfine**2 + m_ls*xfine + b_ls, 'y', label="Least Square")
...
result = op.minimize(nll, [a_true,m_true, b_true, np.log(f_true)], args=(x,y,yerr)) #function, initial guesses, additional arguments passed to objective (nll) function
a_ml, m_ml, b_ml, lnf_ml = result["x"] #this result["x"] gives m,b, lnf in one umbrella term
plt.plot(xfine, parab(xfine, a_ml, m_ml, b_ml), 'm-', label="Max Likelihood")
print("Max liklihood a is", a_ml)
print(m_ml)
print(b_ml)
plt.title("Plot of Least Square and Maximized Likelihood Fits")
plt.legend()
...
def lnprior(theta):            #log prior
    a, m, b, lnf = theta
    if 0 < a < 5 and -5.0 < m < 0.5 and 0.0 < b < 10.0 and -10.0 < lnf < 1.0:
        return 0.0
...
ndim, nwalkers = 4, 100         #no. dimensions, no. walkers
...
fig, ax = plt.subplots(4,1, sharex = True) #conveniently packs fig ang subplots in 1, sharex = share x axes
for i in [0,1,2,3]: #note must specify all numbers here as no range specified
    ...
    plt.suptitle("Random walker position for a, m, b, lnf as a function of steps") #puts title at top
...
fig = corner.corner(samples, labels=["$a$", "$m$", "$b$", "$\ln\,f$"], 
                    truths = [a_true, m_true, b_true, np.log(f_true)])
...
plt.figure(4)
for a, m, b, lnf in samples[np.random.randint(len(samples), size = 100)]: #if high isnt specified results are from 0 - low (ie len(samples))
    plt.plot(xfine, a*xfine**2 + m*xfine+b, "k", alpha = 0.1)
plt.plot(xfine, a_true*xfine**2 + m_true*xfine + b_true, "r", lw = 2, alpha = 0.8)
plt.errorbar(x, y , yerr, fmt=".k", capsize = 1, lw=0.5)
